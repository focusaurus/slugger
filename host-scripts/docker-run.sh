...
# debian: packages
RUN apt-get -qq -y update >/dev/null; apt-get -qq -y install curl git less wget vim
...
EXPOSE 9999
ENV PATH /usr/local/sbin:/usr/local/bin:/usr/local/cargo/bin:/usr/sbin:/usr/bin:/sbin:/bin:/host/bin
WORKDIR /host
